    ...
    <div class="card-body">
        <form action="{{ route("admin.crops.store") }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="form-group {{ $errors->has('image') ? 'has-error' : '' }}">
                ...
                <input type="file" min="1" id="image" name="image" class="form-control" value="{{ old('image', isset($crops) ? $crops->image : '') }}" required>
                @if($errors->has('image'))
                    <em class="invalid-feedback">
                        {{ $errors->first('image') }}
                    </em>
            ...
            <div class="form-group {{ $errors->has('filename') ? 'has-error' : '' }}">
                ...
                <input type="file" min="1" id="filename" name="filename" class="form-control" value="{{ old('filename', isset($crops) ? $crops->filename : '') }}" required>
